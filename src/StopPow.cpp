...
double StopPow::Eout(double E, double x)
{
    // sanity checking:
    ...
    double x0 = 0.;
    double x1 = x; // range of ODE integration
    double y[1] = {E};
    int status;
    try
    {
...
double StopPow::Ein(double E, double x)
{
    // sanity checking:
    if (E < get_Emin() || E > get_Emax() || x < 0)
    {
...
double StopPow::Thickness(double E1, double E2)
{
    // sanity checking:
    if (E1 < get_Emin() || E1 > get_Emax() || E2 < get_Emin() || E2 > get_Emax() || E2 > E1)
    {
        std::stringstream msg;
