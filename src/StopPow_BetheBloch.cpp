...
*/
StopPow_BetheBloch::StopPow_BetheBloch(double mt_in,
                                       double Zt_in,
                                       ...
                                       std::vector<double> nf_in)
{
    ...
    if (mt_in <= 0 || Zt_in <= 0 || Zf_in.size() != num || nf_in.size() != num)
    {
        args_ok = false;
    }
    ...
    if (!args_ok)
    {
        ...
        msg << "Values passed to StopPow_LP constructor are bad: " << mt_in << "," << Zt_in << ","
            << std::endl;
 ...
 * @throws invalid_argument
*/
double StopPow_BetheBloch::dEdx_MeV_um(double E)
{
    // sanity check:
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        prefac = 4.0 * M_PI * Na * rho * pow(Zt * e * e, 2) * Zf[i]
                 / (me * c * c * beta * beta * mf[i]);
 ...
 * @throws invalid_argument
 */
double StopPow_BetheBloch::dEdx_MeV_mgcm2(double E)
{
...
void StopPow_BetheBloch::set_Ibar(std::vector<double> Ibar)
{
    ...
    else
    {
        throw std::invalid_argument(
                "StopPow_BetheBloch::set_Ibar got wrong number of elements passed to it");
    }
}
