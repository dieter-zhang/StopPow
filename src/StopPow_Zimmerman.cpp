...
StopPow_Zimmerman::StopPow_Zimmerman(double mt,
                                     double Zt,
                                     std::vector<std::array<double, 5>>& field,
                                     double Te_in)
...
double StopPow_Zimmerman::dEdx_MeV_um(double E)
{
    // sanity check:
    if (E < Emin || E > Emax)
    {
...
double StopPow_Zimmerman::dEdx_MeV_mgcm2(double E)
{
...
void mu_fdf(double x, void* params, double* y, double* dy)
{
    *y = mu_f(x, params);
    *dy = mu_df(x, params);
}
...
double StopPow_Zimmerman::dEdx_free_electron(double E)
{
    ...
    if (quantum)
    {
        ...
        vth = fmax(vth,
                   (h / (2. * sqrt(M_PI) * me))
                           * pow(4 * ne * (1 + exp(-mu / (kB * Te * keVtoK))), 1. / 3));
    ...
    // Eq 16:
    double LambdaF = (4 * M_PI * me * pow(vth, 2) / (h * omega_pe))
                     * (0.321 + 0.259 * pow(y, 2) + 0.0707 * pow(y, 4) + 0.05 * pow(y, 6))
                     / (1. + 0.130 * pow(y, 2) + 0.05 * pow(y, 4));
    double dEdx_F =
            4 * M_PI * (1. / me) * pow((double)esu, 4) * pow(Zt / vt, 2) * ne * LF(y, LambdaF);
...
double StopPow_Zimmerman::dEdx_ion(double E)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        bi = sqrt(pow(h / (4 * M_PI * mr * vt), 2)
                  + pow(esu * esu * Zf[i] * Zt / (mr * vt * vt), 2));
