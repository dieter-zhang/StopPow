...
StopPow_Plasma::StopPow_Plasma(double mt_in,
                               double Zt_in,
...
{
    // sanity check
    if (mt_in <= 0 || isnan(mt_in) || Zt_in <= 0 || isnan(Zt_in))
    {
        std::stringstream msg;
        // start constructing message, add info on mt and Zt:
        msg << "Values passed to StopPow_Plasma::set_particle are bad: " << mt_in << "," << Zt_in
            << "," << std::endl;
...
void StopPow_Plasma::set_field(std::vector<double>& mf_in,
                               std::vector<double>& Zf_in,
                               std::vector<double>& Tf_in,
                               std::vector<double>& nf_in)
{
    ...
    if (Zf_in.size() != num || Tf_in.size() != num || nf_in.size() != num)
    {
        args_ok = false;
    }
