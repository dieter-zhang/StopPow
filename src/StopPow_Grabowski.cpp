...
StopPow_Grabowski::StopPow_Grabowski(double mt_in,
                                     double Zt_in,
...
StopPow_Grabowski::StopPow_Grabowski(double mt,
                                     double Zt,
...
double StopPow_Grabowski::dEdx_field(double E, int i)
{
    ...
    ret += (-R(w, g, s, Zt) * (G(w) * log(pow(M_E, .5) + (alpha + w * w) / G_g0) + H(w))) * norm
           * 624150.934; // MeV/cm
...
{
    return s * log(1. + alpha * pow(M_E, -0.5) / (g * (1 + G_a * Z * Z * g)))
           / log(1. + alpha * pow(M_E, -0.5) / G_g0);
}
...
{
    return (M1(g, s, Z) + G_b * M2(w, g, s) * pow(w, 2)) * pow(1 + g, 2 / 3)
           / (w * w * (1. + G_b * w * w));
}
...
{
    return pow(w, 4.) * log(w) / (12. + pow(w, 4))
           - pow(w, 3) * exp(-w * w / 2.) / (3 * sqrt(2 * M_PI));
}
