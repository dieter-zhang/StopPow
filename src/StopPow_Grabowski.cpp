...
StopPow_Grabowski::StopPow_Grabowski(double mt_in, double Zt_in, std::vector<double> & mf_in, std::vector<double> & Zf_in, std::vector<double> & Tf_in, std::vector<double> & nf_in)
	: StopPow_Plasma(mt_in, Zt_in, mf_in, Zf_in, Tf_in, nf_in)
{
	init();
}
StopPow_Grabowski::StopPow_Grabowski(double mt, double Zt, std::vector< std::array<double,4> > & field)
	: StopPow_Plasma(mt, Zt, field)
{
	init();
}
StopPow_Grabowski::StopPow_Grabowski(double mt_in, double Zt_in, std::vector<double> & mf_in, std::vector<double> & Zf_in, std::vector<double> & Tf_in, std::vector<double> & nf_in, double Te)
	: StopPow_Plasma(mt_in, Zt_in, mf_in, Zf_in, Tf_in, nf_in, Te)
{
	init();
}
StopPow_Grabowski::StopPow_Grabowski(double mt, double Zt, std::vector< std::array<double,4> > & field, double Te)
...
double StopPow_Grabowski::dEdx_MeV_um(double E)
{
...
// Calculate the total stopping power
double StopPow_Grabowski::dEdx_MeV_mgcm2(double E)
{
	return (dEdx_MeV_um(E)*1e4) / (rho*1e3);
}

// Get stopping power due only to a specific field particle species
double StopPow_Grabowski::dEdx_field(double E, int i)
{
	// sanity check:
	if( E < Emin || E > Emax )
	{
