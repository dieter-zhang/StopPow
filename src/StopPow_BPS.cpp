...
StopPow_BPS::StopPow_BPS(double mt_in,
                         double Zt_in,
                         std::vector<double>& mf_in,
                         std::vector<double>& Zf_in,
                         std::vector<double>& Tf_in,
                         std::vector<double>& nf_in)
        : StopPow_Plasma(mt_in, Zt_in, mf_in, Zf_in, Tf_in, nf_in)
{
    init();
}

StopPow_BPS::StopPow_BPS(double mt, double Zt, std::vector<std::array<double, 4>>& field)
...
StopPow_BPS::StopPow_BPS(double mt, double Zt, std::vector<std::array<double, 4>>& field, double Te)
...
double StopPow_BPS::dEdx_MeV_um(double E)
{
    // sanity check:
    if (E < Emin || E > Emax)
    {
    ...
    auto shortF = [this, &E, &ret_short]()
    {
        ret_short = this->dEdx_short(E);
    };
    std::thread t1(shortF);
    auto longF = [this, &E, &ret_long]()
    {
        ret_long = this->dEdx_long(E);
    };
    std::thread t2(longF);
    auto quantumF = [this, &E, &ret_quantum]()
    {
        ret_quantum = this->dEdx_quantum(E);
    };
...
// Calculate the total stopping power
double StopPow_BPS::dEdx_MeV_mgcm2(double E)
{
    return (dEdx_MeV_um(E) * 1e4) / (rho * 1e3);
}

// Get stopping power due only to a specific field particle species
double StopPow_BPS::dEdx_field(double E, int i)
{
    double ret_short, ret_long, ret_quantum; // return values for the terms

    // Use threading to speed up:
    auto shortF = [this, &E, &ret_short, &i]()
    {
        ret_short = this->dEdx_short(E, i);
    };
    std::thread t1(shortF);
    auto longF = [this, &E, &ret_long, &i]()
    {
        ret_long = this->dEdx_long(E, i);
    };
    std::thread t2(longF);
    auto quantumF = [this, &E, &ret_quantum, &i]()
    {
        ret_quantum = this->dEdx_quantum(E, i);
    };
    std::thread t3(quantumF);
...
{
    struct int_params* p =
            (struct int_params*)params; // for GSL-style params, have to cast from void*
    double term1 = sqrt(u) * exp(-0.5 * p->beta_b * p->mb * pow(p->vp, 2) * u);
    double term2 = ((-log(p->beta_b * (fabs(p->ep * p->eb) * p->K / (4. * M_PI)) * (p->mb / p->mpb)
                          * u / (1. - u))
                     + 2. - 2. * 0.5772)
                            * (p->beta_b * p->Mpb * pow(p->vp, 2) - 1. / u)
                    + 2. / u);
    return term1 * term2;
}
...
double StopPow_BPS::dEdx_short(double E, int i)
{
    ...
    struct int_params params = {beta_b[i],
                                vp,
                                Zt * e_LH,
                                Zf[i] * e_LH,
                                kappa_b[i],
                                ...
                                mpb[i],
                                Mpb[i]};
    prefac = (pow(Zt * e_LH, 2) / (4. * M_PI)) * (pow(kappa_b[i], 2) / (mt * amu * vp))
             * sqrt(mf[i] * amu / (2. * M_PI * beta_b[i]));
    gsl_function Fa;
...
gsl_complex StopPow_BPS::dEdx_long_func(double vp, double x, int i)
{
    ...
    // prefactor:
    double part1 = (pow(Zt * e_LH, 2) / (8 * M_PI * M_PI)) * x
                   * (rho_b(vp * x, i) / rho_tot(vp * x)); // this part is nice and real
...
double StopPow_BPS::dEdx_long(double E, int i)
{
    ...
    dEdx_cR_1 =
            gsl_complex_mul(dEdx_cR_1, gsl_complex_rect(624150.934 * 1e-4, 0)); // Convert to MeV/um

    // Evaluation of the second term of the equation
    gsl_complex prefac2 = gsl_complex_rect(0,
                                           (pow(Zt * e_LH, 2) / (8 * M_PI * M_PI))
                                                   * (1. / (beta_b[i] * mt * amu * pow(vp, 2)))
    ...
    gsl_complex term1 = gsl_complex_mul(
            Fv, gsl_complex_log(gsl_complex_div(Fv, gsl_complex_rect(pow(K, 2), 0))));
    gsl_complex term2 = gsl_complex_mul(
            Fvc, gsl_complex_log(gsl_complex_div(Fvc, gsl_complex_rect(pow(K, 2), 0))));
    gsl_complex term = gsl_complex_sub(term1, term2);
    term = gsl_complex_mul(term, prefac2);
    // final result:
    gsl_complex dEdx_cR_2 =
            gsl_complex_mul(term, gsl_complex_rect(624150.934 * 1e-4, 0)); // Convert to MeV/um
...
double dEdxQ_func(double vpb, void* params)
{
    ...
    double term2a =
            (1.
             + ((p->Mpb * p->vp) / (p->mb * vpb)) * (1. / (p->beta_b * p->mb * p->vp * vpb) - 1.))
...
double StopPow_BPS::dEdx_quantum(double E, int i)
{
    ...
    // prefactor:
    prefac = (pow(Zt * e_LH, 2) / (4 * M_PI))
             * (pow(kappa_b[i], 2) / (2 * beta_b[i] * mt * amu * pow(vp, 2)))
             * sqrt(beta_b[i] * mf[i] * amu / (2 * M_PI));
    ...
    auto etaFunc = [this, &i](double vpb)
    {
        return this->eta_pb(vpb, i);
    };
    // parameters for numerical integration
    struct quantum_params params = {beta_b[i],
                                    vp,
                                    Zt * e_LH,
                                    Zf[i] * e_LH,
                                    kappa_b[i],
