...
// constructors
StopPow_Mehlhorn::StopPow_Mehlhorn(double mt_in,
                                   double Zt_in,
...
StopPow_Mehlhorn::StopPow_Mehlhorn(double mt,
                                   double Zt,
 ...
 */
double StopPow_Mehlhorn::dEdx_MeV_um(double E)
{
    // sanity check:
    if (E < Emin || E > Emax)
    {
 ...
 * @throws invalid_argument
 */
double StopPow_Mehlhorn::dEdx_MeV_mgcm2(double E)
{
...
double StopPow_Mehlhorn::dEdx_nuc(double E, int index)
{
    ...
    double Cnprime = mf[index] * mt / (mf[index] + mt) * (1. / (ZtEff(E) * Zf[index]))
                     / sqrt(pow(ZtEff(E), 2 / 3) + pow(Zf[index], 2 / 3));
...
double StopPow_Mehlhorn::dEdx_Bethe(double E, int index)
{
    ...
    prefac = 4.0 * M_PI * Na * rho_i * pow(ZtEff(E) * e * e, 2) * (Zf[index] - Zbar[index])
             / (me * c * c * beta * beta * mf[index]);
...
// Set manual Ibars:
void StopPow_Mehlhorn::set_Ibar(std::vector<double> Ibar)
{
    ...
    else
    {
        throw std::invalid_argument(
                "StopPow_Mehlhorn::set_Ibar got wrong number of elements passed to it");
    }
}
