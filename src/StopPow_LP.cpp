...
StopPow_LP::StopPow_LP(double mt_in,
                       double Zt_in,
...
StopPow_LP::StopPow_LP(double mt, double Zt, std::vector<std::array<double, 4>>& field)
...
StopPow_LP::StopPow_LP(double mt, double Zt, std::vector<std::array<double, 4>>& field, double Te_in)
...
double StopPow_LP::dEdx_MeV_um(double E)
{
...
double StopPow_LP::dEdx_field(double E, int i)
{
    ...
    if (collective)
    {
        ...
        else
        {
            ...
            double LogLambdaC = gsl_sf_bessel_K0(xInvSqrt) * gsl_sf_bessel_K1(xInvSqrt) * xInvSqrt;
...
double StopPow_LP::Tq(int index)
{
    if (quantumT)
    {
        ...
        double TF = (1 / kB) * ((double)hbar * (double)hbar / (2. * mf[index] * amu))
                    * pow(3 * M_PI * M_PI * nf[index], 2. / 3.);
        double theta = Tf[index] * keVtoK / TF;
        // chemical potential over kT: Drake Eq 3.20
        double mukT = -1.5 * log(theta) + log(4. / (3. * sqrt(M_PI)))
                      + (0.25054 * pow(theta, -1.858) + 0.072 * pow(theta, -1.858 / 2))
                                / (1 + 0.25054 * pow(theta, -0.858));
