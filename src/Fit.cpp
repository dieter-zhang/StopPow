...
int gauss_df(const gsl_vector* p, void* data, gsl_matrix* J)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        double dGdmu = (A / (sqrt(2 * M_PI) * pow(sigma, 3)))
                       * exp(-1. * pow(x[i] - mu, 2) / (2. * sigma * sigma)) * (x[i] - mu);
                           ...
                           bool verbose)
{
    ...
    gsl_multifit_covar(J, 1e-4, covar);
    gsl_matrix_free(J);
    ...
    if (verbose)
    {
        printf("chisq/dof = %g\n", pow(chi, 2.0) / dof);
        printf("A      = %.5f +/- %.5f\n",
               gsl_vector_get(s->x, 0) * scale,
               sqrt(gsl_matrix_get(covar, 0, 0)) * scale);
        printf("mu     = %.5f +/- %.5f\n",
               gsl_vector_get(s->x, 1),
               sqrt(gsl_matrix_get(covar, 1, 1)));
        printf("sigma  = %.5f +/- %.5f\n",
               gsl_vector_get(s->x, 2),
               sqrt(gsl_matrix_get(covar, 2, 2)));
        printf("status = %s\n", gsl_strerror(status));
    }
                       ...
                       bool verbose)
{
    ...
    try
    {
        ...
        rhoR_unc =
                sqrt(0.25 * pow(rhoR_max1 - rhoR_min1, 2) + 0.25 * pow(rhoR_max2 - rhoR_min2, 2));
...
int forward_gauss_f(const gsl_vector* p, void* data, gsl_vector* f)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        double y_eval =
                (A / (sqrt(2 * M_PI) * sigma)) * exp(-1. * pow(Ein - E0, 2) / (2 * pow(sigma, 2)));
                               ...
                               bool verbose)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        gsl_multifit_fdfsolver_jac(solver, J2);
        gsl_multifit_covar(J2, 1e-4, covar);
        gsl_matrix_free(J2);
    ...
    if (verbose)
    {
        ...
        printf("rhoR   = %.5f +/- %.5f\n",
               ...
               sqrt(gsl_matrix_get(covar, 0, 0)));
        printf("A      = %.5f +/- %.5f\n",
               gsl_vector_get(solver->x, 1) * scale,
               sqrt(gsl_matrix_get(covar, 1, 1)) * scale);
        printf("sigma  = %.5f +/- %.5f\n",
                                  ...
                                  bool verbose)
{
    ...
    double drhoR_2 = fabs(results[3] - results[2]) / 2.;
    double rhoR_unc =
            sqrt(pow(drhoR_1, 2) + pow(drhoR_2, 2) + pow(x_hi - x_lo, 2) + pow(results_unc[4], 2));
...
int forward_dEdx_f(const gsl_vector* p, void* data, gsl_vector* f)
{
    ...
    for (size_t i = 0; i < d->x.size(); i++)
    {
        ...
        y_eval = (A / (sqrt(2 * M_PI) * d->sigma0))
                 * exp(-1. * pow(Ein - d->E0, 2) / (2 * pow(d->sigma0, 2)));
        // correct  for "accordion" effect on spectrum:
                               ...
                               bool verbose)
{
    ...
    std::vector<double> vary_sigma0{
            sigma, sigma, sigma, sigma, sigma - sigma_unc, sigma + sigma_unc, sigma, sigma, sigma};
    std::vector<double> vary_rhoR{
            rhoR, rhoR, rhoR, rhoR, rhoR, rhoR, rhoR + rhoR_unc, rhoR - rhoR_unc, rhoR};
    for (int i = 0; i < 9; i++) // loop corresponds to varying above params
    {
        ...
        // set up the data for fitting:
        struct ff_dEdx_data d = {
                data_x2, data_y2, data_std2, vary_E0[i], vary_sigma0[i], vary_rhoR[i], &s};
        ...
        gsl_multifit_fdfsolver_jac(solver, J3);
        gsl_multifit_covar(J3, 1e-3, covar);
        gsl_matrix_free(J3);
        chi = gsl_blas_dnrm2(solver->f);
        dof = n - p;
    ...
    fit_unc.push_back(sqrt(pow(dfactor_1, 2) + pow(dfactor_2, 2) + pow(dfactor_3, 2)
                           + pow(dfactor_4, 2) + gsl_matrix_get(covar, 0, 0)));
