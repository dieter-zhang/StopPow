...
StopPow_PartialIoniz::StopPow_PartialIoniz(double mt_in,
                                           double Zt_in,
...
void StopPow_PartialIoniz::set_particle(double mt_in, double Zt_in)
{
    ...
    if (mt_in <= 0 || isnan(mt_in) || Zt_in <= 0 || isnan(Zt_in))
    {
        ...
        msg << "Values passed to StopPow_PartialIoniz::set_particle are bad: " << mt_in << ","
            << Zt_in << "," << std::endl;
...
void StopPow_PartialIoniz::set_field(std::vector<std::array<double, 5>>& field, double Te)
{
...
// Method to set field particle info
void StopPow_PartialIoniz::set_field(std::vector<double>& mf_in,
                                     std::vector<double>& Zf_in,
                                     std::vector<double>& Tf_in,
                                     std::vector<double>& nf_in,
                                     std::vector<double>& Zbar_in,
                                     double Te_in)
{
    ...
    if (Zf_in.size() != num || Tf_in.size() != num || nf_in.size() != num || Zbar_in.size() != num)
    {
        args_ok = false;
    }
